...
Texture2DDX12::Texture2DDX12(DX12Renderer* renderer)
{
	this->renderer = renderer;
	this->device = renderer->Getdevice5();
	resourceDescription = {};
...
{
	// Get image size and setup resource description
	int imageSize = loadTextureFromFile(filename);
	...
	// Get heap size (int textureHeapSize = ((((width * numBytesPerPixel) + 255) & ~255) * (height - 1)) + (width * numBytesPerPixel);)
	// TODO: Fatta formel
	...
	heapProperties2.Type = D3D12_HEAP_TYPE_UPLOAD;
	heapProperties2.CPUPageProperty = D3D12_CPU_PAGE_PROPERTY_UNKNOWN;
	heapProperties2.CreationNodeMask = 1; //used when multi-gpu
	heapProperties2.VisibleNodeMask = 1; //used when multi-gpu
	...
	// reset list and queue
	renderer->clearBuffer(0);

	// Now we copy the upload buffer contents to the default heap
	UpdateSubresources(renderer->GetCommandList(), textureBuffer, textureBufferUploadHeap, 0, 0, 1, &textureData);

	// transition the texture default heap to a pixel shader resource (we will be sampling from this heap in the pixel shader to get the color of pixels)
	// TODO: Stefan förklara
	renderer->GetCommandList()->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
			textureBuffer,
			D3D12_RESOURCE_STATE_COPY_DEST,
	...
	device->CreateShaderResourceView(textureBuffer, &srvDesc, renderer->GetDH()->GetCPUDescriptorHandleForHeapStart());


	renderer->GetCommandList()->Close();
	ID3D12CommandList* ppCommandLists[] = { renderer->GetCommandList() };
	renderer->GetCommandQueue()->ExecuteCommandLists(ARRAYSIZE(ppCommandLists), ppCommandLists);
...
int Texture2DDX12::loadTextureFromFile(std::string filename)
{
	...
	// TODO: I vilket fall skall vi inte det
